...
import { useEffect, useState, useMemo, memo, FocusEvent } from 'react';
import { DateTimeToString, formatRangeDate } from 'src/utils/dateUntils';
...
import useAddDayTimeBooking from 'src/hooks/guest/timeBooking/useAddDayTimeBooking';
...
const DaySection = () => {
    ...
    const [resValue, setResValue] = useState<IDayBookResponse>({ content: [] });
    ...
    const { addDayTimeBooking, isLoadingPost } = useAddDayTimeBooking(id as string, dateRange.startDate, dateRange.endDate);
    ...
    return (
        ...
        >
            <Box p={6}>
                <Heading lineHeight={1.4} as="h2" fontSize={'26px'} fontWeight={'600'} width={'full'} noOfLines={2} pb={6}>
            ...
            <Box py={6}>
                <Heading lineHeight={1.4} as="h3" fontSize={'18px'} fontWeight={'600'} width={'full'} noOfLines={3} px={6} pb={4}>
                    {resValue.content.length > 0
                        ? 'Ngày và các khung thời gian'
                        : 'Các ngày bạn chọn không có sẵn, hãy nhấn vào nút bên dưới để thêm ngày mới'}
                </Heading>
                <Flex justifyContent={'space-between'}>
                    <Accordion allowToggle w={'full'}>
                        {isFetching || isLoading || isLoadingPost ? (
                            ...
                            <Button
                                ...
                                colorScheme={'teal'}
                                onClick={() => {
                                    addDayTimeBooking({ start_time: dateRange.startDate, end_time: dateRange.endDate });
                                }}
                            >
